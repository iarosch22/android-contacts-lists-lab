...
public class FilterContactTypeViewModel extends BaseBottomSheetViewModel {
    ...
    private void updateFilterContactTypes() {
        String[] contactTypeList = ContactType.getContactTypes();

        ArrayList<FilterContactTypeUi> filterContactsList = new ArrayList<>();
        filterContactsList.add(createAllSelectedItem(contactTypeList));
        for (String contact: contactTypeList) {
            filterContactsList.add(createFilterContactType(contact));
        }
        ...
        filterContactTypesLiveDate.setValue(filterContactsList);
    }
